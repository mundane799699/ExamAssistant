...
public class OptionMultiRvAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        if (!mQuestion.getIsShowAnswer()) { // 答题模式
            if (holder instanceof FootViewHolder) {    //	footer
                ...
                if (mQuestion.getHaveBeenAnswered()) {
                    ...
                    footViewHolder.mBtnSubmit.setVisibility(View.VISIBLE);
                }
            ...
            if (holder instanceof NormalViewHolder) {
                ...
                switch (position) {
                    case 0:
                        normalViewHolder.mTvOption.setText(mQuestion.getOptionA());
                        setShowAnswerStatus(mQuestion.getIsOptionACorrect(), normalViewHolder, position);
                        break;
                    case 1:
                        normalViewHolder.mTvOption.setText(mQuestion.getOptionB());
                        setShowAnswerStatus(mQuestion.getIsOptionBCorrect(), normalViewHolder, position);
                        break;
                    case 2:
                        normalViewHolder.mTvOption.setText(mQuestion.getOptionC());
                        setShowAnswerStatus(mQuestion.getIsOptionCCorrect(), normalViewHolder, position);
                        break;
                    case 3:
                        normalViewHolder.mTvOption.setText(mQuestion.getOptionD());
                        setShowAnswerStatus(mQuestion.getIsOptionDCorrect(), normalViewHolder, position);
                        break;
                    case 4:
                        normalViewHolder.mTvOption.setText(mQuestion.getOptionE());
                        setShowAnswerStatus(mQuestion.getIsOptionECorrect(), normalViewHolder, position);
                    default:
                        break;
                }
            } else { // footViewHolder
    ...
    private void setShowAnswerStatus(boolean isOptionCorrect, NormalViewHolder holder, int position) {
        if (isOptionCorrect) {
